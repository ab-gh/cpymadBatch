    ...
    def __init__(self, 
        ...
        Initialise a configuration for a cpymad batch job.
            ...
            Parameters:
                ...
                output_dir (str): Path to the output directory (default: /eos/user/<u>/<username>/cpymadbatch/<name>)
        ...
        while True:
            if os.path.exists(f"/eos/user/{self.user[0]}/{self.user}/cpymadbatch/{name}"):
                ...
                self.output_dir = output_dir if output_dir else f"/eos/user/{self.user[0]}/{self.user}/cpymadbatch/{self.name + str(i)}/output"
                break
        self.local = local
